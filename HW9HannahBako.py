...
def main():
    student_table = PrettyTable(['CWID', 'Name', 'Major', 'Course(s)'])
    instructor_table = PrettyTable(['CWID', 'Name', 'Department', 'Student(s)'])
    new_repo = Repo()
    new_repo.readstudent()
    new_repo.readinstructor()
    new_repo.readgrades()
    for student in new_repo.dict_student.values():
        student_table.add_row([student.cwid, student.name, student.major, list(student.courses.keys())])
    for instructor in new_repo.dict_instructor.values():
        instructor_table.add_row([instructor.cwid, instructor.name, instructor.department, len(instructor.courses)])
    print(student_table)
